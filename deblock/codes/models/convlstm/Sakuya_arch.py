...
from models.convlstm.convlstm import ConvLSTM, ConvLSTMCell
try:
...
class Easy_PCD(nn.Module):
    def __init__(self, nf=64, groups=8, nf_in=24):
        super(Easy_PCD, self).__init__()

        self.fea_L1_conv1 = nn.Conv2d(nf, nf_in, 3, 1, 1, bias=True)
        self.fea_L2_conv1 = nn.Conv2d(nf_in, nf_in, 3, 2, 1, bias=True)
        self.fea_L2_conv2 = nn.Conv2d(nf_in, nf_in, 3, 1, 1, bias=True)
        self.fea_L3_conv1 = nn.Conv2d(nf_in, nf_in, 3, 2, 1, bias=True)
        self.fea_L3_conv2 = nn.Conv2d(nf_in, nf_in, 3, 1, 1, bias=True)
        self.pcd_align = PCD_Align(nf=nf_in, groups=groups)
        self.fusion = nn.Conv2d(2 * nf_in, nf, 1, 1, bias=True)
        self.lrelu = nn.LeakyReLU(negative_slope=0.1, inplace=True)
...
class DeformableConvLSTM(ConvLSTM):
    ...
    def forward(self, input_tensor, hidden_state = None):
        ...
        return layer_output_list[:, seq_len // 2, :, :, :]
...
class BiDeformableConvLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        B, C, H, W = out_fwd.size()
        result = torch.cat((out_fwd, out_rev), dim=1)
...
class LunaTokis(nn.Module):
    def __init__(self, nf=64, nframes=3, groups=8, front_RBs=5, back_RBs=10):
        ...
        self.HRconv = nn.Conv2d(nf, 64, 3, 1, 1, bias=True)
