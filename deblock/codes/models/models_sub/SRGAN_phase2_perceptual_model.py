...
class SRGANPhase2PerceptualModel(BaseModel):
    def __init__(self, opt):
        super(SRGANPhase2PerceptualModel, self).__init__(opt)
        if opt['dist']:
        ...
        if 'w_phase1' in self.opt['network_G'].keys():
            self.w_phase1 = self.opt['network_G']['w_phase1']
        else:
            self.w_phase1 = True
        if self.w_phase1:
    ...
    def optimize_parameters(self, step):
        ...
        if self.one2many:
            z = torch.randn(self.var_L.shape[0], 1, self.var_L.shape[2], self.var_L.shape[3]).to(self.device)
            if self.w_phase1:
                self.fake_H = self.netG(torch.cat((self.netG_phase1(self.var_L), z), dim = 1))
            else:
                self.fake_H = self.netG(torch.cat(self.var_L, z))
        else:
            if self.w_phase1:
        ...
        if step % self.D_update_ratio == 0 and step >= self.D_init_iters:
            ...
            if self.loss_fea:  # perceptual loss
                loss_fea_g, _ = self.loss_fea(self.Output, self.ground_truth)
                loss_fea_g = self.loss_fea_w * loss_fea_g
    ...
    def test(self):
        ...
        with torch.no_grad():
            if self.one2many:
                # z = torch.randn(self.var_L.shape[0], 1, self.var_L.shape[2], self.var_L.shape[3]).to(self.device)
                z = torch.zeros(self.var_L.shape[0], 1, self.var_L.shape[2], self.var_L.shape[3]).to(self.device)
                if self.w_phase1:
                    self.fake_H = self.netG(torch.cat((self.netG_phase1(self.var_L), z), dim=1))
                else:
                    self.fake_H = self.netG(torch.cat(self.var_L, z))
