...
class VideoSemiModelA(BaseModel):
    def __init__(self, opt):
        super(VideoSemiModelA, self).__init__(opt)
    ...
    def feed_data(self, data, need_GT=True):
        self.var_H = data['GTs'].to(self.device)
        # if need_GT:
    ...
    def optimize_parameters(self, step):
        ...
        self.fake_H = self.netG(self.var_H)

        B, N, C, H, W = self.fake_H.shape
        ...
        for i in range(N):
            if i == center:
                continue
            l_pix += self.l_pix_w * self.cri_pix(self.fake_H[:, i, :, :, :], self.var_H[:, center, :, :, :])
    ...
    def get_current_visuals(self, need_GT=True):
        ...
        center = self.var_H.shape[1] // 2
        out_dict['LQ'] = self.var_H.detach()[0, 0, :, :, :].float().cpu()
        out_dict['rlt'] = self.fake_H.detach()[0, 0, :, :, :].float().cpu()
        if need_GT:
            out_dict['GT'] = self.var_H.detach()[0, center, :, :, :].float().cpu()
        return out_dict
