...
class SRGANPhase2MidModel(BaseModel):
    def __init__(self, opt):
        super(SRGANPhase2MidModel, self).__init__(opt)
        if opt['dist']:
        ...
        if opt['dist']:
            ...
            self.netG_phase1 = DataParallel(self.netG_phase1)
        if self.is_train:
    ...
    def optimize_parameters(self, step):
        ...
        self.fake_H_1, self.fake_H = self.netG(self.var_L)
        ...
        if step % self.D_update_ratio == 0 and step >= self.D_init_iters:
            ...
            if self.loss_dis:  # distortion loss
                loss_dis_g_1 = self.loss_dis_w * self.loss_dis(self.fake_H_1, self.ground_truth)
                loss_dis_g_2 = self.loss_dis_w * self.loss_dis(self.Output, self.ground_truth)
                loss_total_g += loss_dis_g_1 + loss_dis_g_2
        ...
        if step % self.D_update_ratio == 0 and step >= self.D_init_iters:
            # G
            if self.loss_dis:
                self.log_dict['loss_dis_g_1'] = loss_dis_g_1.item()
                self.log_dict['loss_dis_g_2'] = loss_dis_g_2.item()
            if self.loss_fea:
