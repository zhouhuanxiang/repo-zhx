...
class VideoMixupModel(BaseModel):
    def __init__(self, opt):
        super(VideoMixupModel, self).__init__(opt)
        ...
        if 'use_mixup' in train_opt.keys():
            self.use_mixup = True
        else:
    ...
    def feed_data(self, data, need_GT=True):
        N, T, C, H, W = data['LQs'].shape
        alpha = random.random()
        self.var_L = data['LQs'][:N // 2, :, :, :, :] * alpha + data['LQs'][N // 2:, :, :, :, :] * (1 - alpha)
        self.var_L = self.var_L.to(self.device)
        if need_GT:
            self.real_H = data['GT'][:N // 2, :, :, :] * alpha + data['GT'][N // 2:, :, :, :] * (1 - alpha)
