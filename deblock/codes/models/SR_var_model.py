...
class SRVarModel(BaseModel):
    ...
    def feed_data(self, data, need_GT=True):
        if self.use_gpu:
            ...
            self.var_L_sigma = data['LQ_sigma'].to(self.device) # LQ_sigma
            if need_GT:
    ...
    def optimize_parameters(self, step):
        ...
        input = torch.cat((self.var_L, self.var_L_sigma), 1)
        self.fake_H = self.netG(input)

        # print(1, torch.mean(torch.abs(self.var_L)))
        # print(2, torch.mean(torch.abs(self.var_L_sigma)))
        # print(3, torch.mean(torch.abs(self.fake_H)))
