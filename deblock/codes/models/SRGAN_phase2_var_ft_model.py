...
class SRGANPhase2VarFtModel(BaseModel):
    def __init__(self, opt):
        super(SRGANPhase2VarFtModel, self).__init__(opt)
        if opt['dist']:
        ...
        self.netG_phase2 = SRResNet_o2m_ft_arch.MSRResNet(in_nc=3, out_nc=3, nf=64, nb=16, upscale=1)
        self.netG = networks.define_G(opt).to(self.device)
        if opt['dist']:
            self.netG = DistributedDataParallel(self.netG, device_ids=[torch.cuda.current_device()])
            self.netG_phase1 = DistributedDataParallel(self.netG_phase1, device_ids=[torch.cuda.current_device()])
            self.netG_phase2 = DistributedDataParallel(self.netG_phase2, device_ids=[torch.cuda.current_device()])
        else:
            self.netG = DataParallel(self.netG).to(self.device)
            self.netG_phase1 = DataParallel(self.netG_phase1).to(self.device)
            self.netG_phase2 = DataParallel(self.netG_phase2).to(self.device)
        if self.is_train:
        ...
        if train_opt['loss_ft_weight']:
            print('############ ft #############')
            self.loss_ft_weight = train_opt['loss_ft_weight']
        else:
            self.loss_ft_weight = 0

        if train_opt['loss_re_weight']:
            print('############ re #############')
            self.loss_re_weight = train_opt['loss_re_weight']
        else:
            self.loss_re_weight = 0

        if self.loss_re_weight > 0:
            self.load_network(
                '/home/web_server/zhouhuanxiang/disk/log/experiments/SRGANx1_phase2_patchls_o2m_KWAI35_2/models/latest_G.pth',
                self.netG_phase2, True)
    ...
    def optimize_parameters(self, step):
        ...
        if self.w_phase1:
            self.var_L = self.netG_phase1(self.var_L)
        ...
        if step % self.D_update_ratio == 0 and step >= self.D_init_iters:
            ...
            if self.loss_tv_weight > 0:
                ...
                loss_total_g += l_tv
            if self.loss_re_weight > 0:
                fake_H_2 = self.netG_phase2(self.var_L)
                l_re = self.loss_re_weight * self.loss_dis(self.fake_H, fake_H_2)
                loss_total_g += l_re
            if self.loss_ft_weight > 0:
                l_ft = self.loss_ft_weight * self.loss_dis(self.fake_H_ft, fake_H_ft_2)
                loss_total_g += l_ft
