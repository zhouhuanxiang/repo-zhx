...
class MSRResNet(nn.Module):
    ...
    def __init__(self, in_nc=3, out_nc=3, nf=64, nb=16, upscale=4,
                 use_var_conv=False, use_var_all_layers=False, use_var_opposite=False):
        super(MSRResNet, self).__init__()
        self.upscale = upscale
        self.use_var_conv = use_var_conv
        self.use_var_all_layers = use_var_all_layers
        self.use_var_opposite = use_var_opposite
        self.nb = nb
        ...
        if self.use_var_conv:
            self.var_conv_1 = nn.Conv2d(1, 1, 3, 1, 1, bias=True)
            self.var_conv_2 = nn.Conv2d(1, 1, 3, 1, 1, bias=True)
    ...
    def forward(self, x, var):
        if self.use_var_opposite:
            var = torch.ones(var.shape).to(x.device) - var
        if self.use_var_conv:
            var = self.var_conv_1(var)
            var = self.var_conv_2(var)
        ...
        for i, layer in enumerate(self.recon_trunk):
            ...
            if self.use_var_all_layers or (not self.use_var_all_layers and i == self.nb // 2):
