...
class DWTForward_shuffle(nn.Module):
    def __init__(self, nf=64):
        super(DWTForward_shuffle, self).__init__()
        self.nf = nf
        self.DWTForward = pytorch_wavelets.DWTForward()
...
class DWTInverse_shuffle(nn.Module):
    def __init__(self, nf=64):
        super(DWTInverse_shuffle, self).__init__()
        self.nf = nf
        self.DWTInverse = pytorch_wavelets.DWTInverse()
...
class EDVR(nn.Module):
    def __init__(self, nf=64, nframes=5, groups=8, front_RBs=5, back_RBs=10, center=None,
        ...
        self.dwt_foward_shuffle = DWTForward_shuffle()
        self.dwt_inverse_shuffle = DWTInverse_shuffle()

    def forward(self, x):
        flip_band = self.training and torch.randint(2, ()).item() > 0

        x_center = x[:, self.center, :, :, :].contiguous()
        x = self.dwt_foward_shuffle(self.dwt_foward(x))
        ...
        out = self.dwt_inverse_shuffle(self.dwt_inverse(out))
