...
class EDVR(nn.Module):
    def __init__(self, nf=64, nframes=5, groups=8, front_RBs=5, back_RBs=10, center=None,
        ...
        if self.w_TSA:
            ...
            self.tsa_fusion = nn.Conv2d(2 * nf, nf, 1, 1, bias=True)
        ...
        self.maxpooling = nn.MaxPool1d(nframes)
        self.avgpooling = nn.AvgPool1d(nframes)
    ...
    def forward(self, x):
        ...
        if not self.w_TSA:
            ...
            aligned_fea = aligned_fea.permute(0, 2, 3, 4, 1).view(b, c * h * w, n)
            aligned_fea_max = self.maxpooling(aligned_fea)
            aligned_fea_max = aligned_fea_max.view(b, c, h, w)
            aligned_fea_avg = self.avgpooling(aligned_fea)
            aligned_fea_avg = aligned_fea_avg.view(b, c, h, w)
            aligned_fea = torch.cat([aligned_fea_avg, aligned_fea_max], 1)
