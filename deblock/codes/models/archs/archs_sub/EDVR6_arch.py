...
class PCD_Align(nn.Module):
    ...
    def __init__(self, nf=64, groups=8, w_GCB=False):
        ...
        self.cbam1 = cbam.CBAM(nf * 2, 16)
        self.cbam2 = cbam.CBAM(nf * 2, 16)
        self.cbam3 = cbam.CBAM(nf * 2, 16)
        self.cbam4 = cbam.CBAM(nf * 2, 16)
        self.cbam5 = cbam.CBAM(nf * 2, 16)
        self.cbam6 = cbam.CBAM(nf * 2, 16)
    ...
    def forward(self, nbr_fea_l, ref_fea_l):
        ...
        '''
        # L3
        L3_offset = self.cbam1(torch.cat([nbr_fea_l[2], ref_fea_l[2]], dim=1))
        ...
        # L2
        L2_offset = self.cbam2(torch.cat([nbr_fea_l[1], ref_fea_l[1]], dim=1))
        ...
        L2_fea = self.lrelu(self.L2_fea_conv(self.cbam3(torch.cat([L2_fea, L3_fea], dim=1))))
        if self.w_GCB:
            L2_fea = self.L2_context_block(L2_fea)
        # L1
        L1_offset = self.cbam4((torch.cat([nbr_fea_l[0], ref_fea_l[0]], dim=1)))
        ...
        L1_fea = self.L1_fea_conv(self.cbam5(torch.cat([L1_fea, L2_fea], dim=1)))
        if self.w_GCB:
            L1_fea = self.L1_context_block(L1_fea)
        # Cascading
        offset = self.cbam6(torch.cat([L1_fea, ref_fea_l[0]], dim=1))
