...
class PCD_Align(nn.Module):
    ...
    def __init__(self, nf=64, groups=8, w_GCB=False):
        ...
        # list1 = list(range(0, nf))
        # list2 = list(range(nf, nf * 2))
        # iters = [iter(list2), iter(list1)]
        # self.concat_index_order = list(itertools.chain(map(next, itertools.cycle(iters)), *iters))
        # print(self.concat_index_order)
    ...
    def forward(self, nbr_fea_l, ref_fea_l):
        ...
        L3_fea = F.interpolate(0.5 * ref_fea_l[2] + 0.5 * L3_fea, scale_factor=2, mode='bilinear', align_corners=False)
        ...
        L2_fea = F.interpolate(0.5 * ref_fea_l[1] + 0.5 *  L2_fea, scale_factor=2, mode='bilinear', align_corners=False)
