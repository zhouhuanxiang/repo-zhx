...
class MSRResNet(nn.Module):
    ...
    def __init__(self, in_nc=3, out_nc=3, nf=64, nb=16, upscale=4):
        ...
        if self.upscale == 2:
            ...
            self.upconv1 = nn.Conv2d(nf, nf * 9, 3, 1, 1, bias=False)
            self.pixel_shuffle = nn.PixelShuffle(3)
        elif self.upscale == 4:
            self.upconv1 = nn.Conv2d(nf, nf * 4, 3, 1, 1, bias=False)
            self.upconv2 = nn.Conv2d(nf, nf * 4, 3, 1, 1, bias=False)
            self.pixel_shuffle = nn.PixelShuffle(2)
        elif self.upscale == 1:
            self.upconv1 = nn.Conv2d(nf, nf, 3, 1, 1, bias=False)

        self.HRconv = nn.Conv2d(nf, nf, 3, 1, 1, bias=False)
