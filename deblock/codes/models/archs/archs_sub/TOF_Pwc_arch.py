...
class TOFPwclow(nn.Module):
    def __init__(self, adapt_official=False):
        super(TOFPwclow, self).__init__()
        ...
        self.conv_first = nn.Conv2d(3 * 7, 64, 3, 1, 1, bias=True)
        basic_block = functools.partial(arch_util.ResidualBlock_noBN, nf=64)
        self.recon_trunk = arch_util.make_layer(basic_block, 10)
    ...
    def forward(self, x):
        ...
        for i in range(7):
            if i == ref_idx:
                ...
                x_nbr = x[:, i, :, :, :]
                flow = self.PwcNet(x_ref, x_nbr)
                flow = self.upsample(flow).permute(0, 2, 3, 1)
                # print(flow.shape)
                x_warped.append(flow_warp(x_nbr, flow * 20))
