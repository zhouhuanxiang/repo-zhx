...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # out = avg_out + max_out
        # print(x.shape)
        out = self.fc2(self.relu1(self.fc1(torch.var(x, (2, 3), keepdim=True))))
        return x + self.sigmoid(out) * x
...
class PCD_Align(nn.Module):
    ...
    def __init__(self, nf=64, groups=8, w_GCB=False):
        ...
        self.L3_offset_conv1 = nn.Conv2d(nf * 3, nf, 3, 1, 1, bias=True)  # concat for diff
        self.L3_offset_conv2 = nn.Conv2d(nf, nf, 3, 1, 1, bias=True)
        self.L3_dcnpack = DCN(nf, nf, 3, stride=1, padding=1, dilation=1, deformable_groups=groups,
                              extra_offset_mask=True)
        # L2: level 2, 1/2 spatial size
        self.L2_offset_conv1 = nn.Conv2d(nf * 3, nf, 3, 1, 1, bias=True)  # concat for diff
...
class EDVR(nn.Module):
    def __init__(self, nf=64, nframes=5, groups=8, front_RBs=5, back_RBs=10, center=None,
        ...
        self.conv_last_1 = nn.Conv2d(64, 64, 3, 1, 1, bias=True)
        self.conv_last_2 = nn.Conv2d(64, 3 * 16, 3, 1, 1, bias=True)
        ...
        # self.channel_attetion1 = ChannelAttention(nf * 4)
        # self.channel_attetion2 = ChannelAttention(nf * 4)
        # self.channel_attetion3 = ChannelAttention(nf)
        # self.channel_attetion4 = ChannelAttention(nf)
    ...
    def forward(self, x):
        ...
        L1_fea = self.lrelu(self.conv_first_1(L1_fea))
        L1_fea = self.dwt_foward(L1_fea)
        # L1_fea = self.channel_attetion1(L1_fea)
        L1_fea = self.lrelu(self.conv_first_2(L1_fea))
        L1_fea = self.dwt_foward(L1_fea)
        # L1_fea = self.channel_attetion2(L1_fea)
        ...
        # out = self.channel_attetion3(out)
        out = self.dwt_inverse(out)
        out = self.conv_last_1(out)
        # out = self.channel_attetion4(out)
        out = self.dwt_inverse(out)
