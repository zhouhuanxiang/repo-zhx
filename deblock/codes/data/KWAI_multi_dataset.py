...
class KWAIMultiDataset(data.Dataset):
    '''
    ...
    def __init__(self, opt):
        super(KWAIMultiDataset, self).__init__()
        self.opt = opt
        ...
        paths = glob.glob(os.path.join(self.GT_root, '*/*.png'))
        self.paths_GT = [p.split('/')[-2] + '_' + p.split('/')[-1].split('.')[0].split('_')[1] for p in paths]
    ...
    def __getitem__(self, index):
        ...
        # else:
        img_GT = util.read_img(None, osp.join(self.GT_root, name_a, 'img_' + name_b + '.png'))

        #### get LQ images
        # LQ_size_tuple = (3, 180, 320) if self.LR_input else (3, 720, 1280)
        img_LQ_l = []
        for v in neighbor_list:
            img_LQ_path = osp.join(self.LQ_root, name_a, 'img_{:05d}.png'.format(v))
            # if self.data_type == 'mc':
            #     if self.LR_input:
