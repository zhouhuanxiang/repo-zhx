...
class LQGTVmafDataset(data.Dataset):
    """
    Read LQ (Low Quality, e.g. LR (Low Resolution), blurry, etc) and GT image pairs.
    If only GT images are provided, generate LQ images on-the-fly.

    only for vmaf pretrain
    """

    def __init__(self, opt):
        super(LQGTVmafDataset, self).__init__()
        self.opt = opt
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            ...
            vmaf_score = util.run_vmaf(img_LQ, img_GT, '{}_{}_{}'.format(index, rnd_h, rnd_w))
        ...
        return {'LQ': img_LQ, 'GT': img_GT, 'LQ_path': LQ_path, 'GT_path': GT_path, 'IQA': vmaf_score}
